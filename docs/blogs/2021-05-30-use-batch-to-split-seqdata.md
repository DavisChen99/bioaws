...
于是是所有的样本就都开始在云端开始拆分，数据放到指定的nas上，并紧接着开始做二级分析出报告。看到这里估计很多人一脸懵，这很正常，如果你都看到了，那么就不要浪费时间再往下看了，饮茶先啦。
...
机型随意选择，我默认选了 _t2.micro_, 启动以后登录进去，做两件事：
...
做完这些事儿以后，将这台机器打个新的AMI镜像为 _ami-123456789_, 聪明的小伙伴可能就想到了，这个新AMI起来的机器，再搭配上上一步设置的 启动模板，就能做到启动后自动挂载lustre和nvme磁盘了。
...
**注意**： s3上写好的脚本命令如下，仅仅是给大家一个参考而已，另外把脚本提前拷贝到容器内再打成新镜像，并不是一个最佳的做法，仅仅针对向split这种比较固化的流程。最好是把脚本放到s3，每次启动分析任务的时候再去拷贝脚本到容器内，这样后期维护脚本也会非常方便。
...
写到此处仅当抛砖引玉，自动化整合大门已经打开，结合S3的event监测，以及无服务器函数服务 Lambda，未来无限可能~
